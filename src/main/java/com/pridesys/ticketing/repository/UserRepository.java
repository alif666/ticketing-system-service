...
public interface UserRepository extends JpaRepository<UserEntity, Long> {
    ...
    @Query("select u from UserEntity u where u.clientId=:client and (lower(u.email) like lower(concat('%',:q,'%')) or lower(u.name) like lower(concat('%',:q,'%')))") Page<UserEntity> searchByClient(@org.springframework.data.repository.query.Param("client") Long client, @org.springframework.data.repository.query.Param("q") String q, Pageable pageable);
}
