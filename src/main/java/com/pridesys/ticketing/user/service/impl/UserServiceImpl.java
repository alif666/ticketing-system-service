package com.pridesys.ticketing.user.service.impl;
...
public class UserServiceImpl implements IUserService {
    ...
    public PageResponse<ProfileResponse> list(UserEntity a, String q, int page, int size) {
        ...
        Page<UserEntity> result = a.getRole() == UserRole.APP_ADMIN ? users.search(q, PageRequest.of(Math.max(0, page), limit)) : users.searchByClient(a.getClientId(), q, PageRequest.of(Math.max(0, page), limit));
        return PageResponse.of(result.map(this::profile));
    }

    public ProfileResponse create(UserEntity a, CreateUserRequest r) {
        admin(a);
        if (a.getRole() == UserRole.CLIENT_ADMIN && r.role() == UserRole.APP_ADMIN)
            throw new AccessDeniedException("Insufficient role");
        Long client = r.clientId();
        if (a.getRole() != UserRole.APP_ADMIN) {
            client = a.getClientId();
            if (r.clientId() != null && !r.clientId().equals(client))
                throw new AccessDeniedException("Outside client scope");
        }
        return profile(users.save(new UserEntity(r.email().trim().toLowerCase(), encoder.encode("Password123!"), r.role(), r.name().trim(), client)));
    }

    public void deactivate(UserEntity a, long id) {
        admin(a);
        var u = target(a, id);
        u.setActive(false);
        users.save(u);
    ...
    private boolean visible(UserEntity a, UserEntity u) {
        return a.getRole() == UserRole.APP_ADMIN || Objects.equals(a.getClientId(), u.getClientId());
    }

    private UserEntity target(UserEntity a, long id) {
        var u = users.findById(id).orElseThrow();
        if (!visible(a, u) || a.getRole() == UserRole.CLIENT_ADMIN && u.getRole() == UserRole.APP_ADMIN)
            throw new AccessDeniedException("Outside client scope");
        return u;
    }
